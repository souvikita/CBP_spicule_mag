...
for time_index in range(96):
	...
	imc1=axs[0].imshow(width[:,:,time_index],origin='lower',vmax=0.82,vmin=0.45,cmap='cividis',extent=[0,1796*0.0379,0,1138*0.0379])
	...
	axs[0].text(.05, .9, r'H$\beta$ LC width [$\AA$]', transform=axs[0].transAxes, fontsize=14,color='black',weight='bold')
	...
	imc2=axs[1].imshow(shift[:,:,time_index]*62,origin='lower',vmax=6,vmin=-6,cmap='RdBu_r',extent=[0,1796*0.0379,0,1138*0.0379])
	...
	axs[1].text(.05, .9, r'V$_{\mathrm{LOS}}$ [km s$^{-1}$]', transform=axs[1].transAxes, fontsize=14,color='black',weight='bold')
	cbar2.ax.xaxis.set_ticks_position("top")
